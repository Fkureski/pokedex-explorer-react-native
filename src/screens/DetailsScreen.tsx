...
export default function DetailsScreen({ route }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setData(d);
      } catch { setError('Não foi possível carregar detalhes.'); }
  ...
  const sprite = data?.sprites?.other?.['official-artwork']?.front_default ?? data?.sprites?.front_default ?? undefined;
    ...
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      <View style={{ alignItems: 'center' }}>
        ...
        <View style={{ marginTop: 8, flexDirection: 'row', gap: 8 }}>
          {types.map((t: string) => <Text key={t} style={{ textTransform: 'capitalize' }}>{t}</Text>)}
        </View>
        <View style={{ marginTop: 12 }}>
          <Button title={isFav(name) ? 'Desfavoritar' : 'Favoritar'} onPress={() => toggleFav(name)} />
        </View>
      </View>

      <View style={{ marginTop: 24 }}>
        <Text style={{ fontWeight: '700', marginBottom: 8 }}>Habilidades</Text>
        {abilities.map((a: string) => <Text key={a} style={{ textTransform: 'capitalize' }}>• {a}</Text>)}
      </View>
    </ScrollView>
  );
}
